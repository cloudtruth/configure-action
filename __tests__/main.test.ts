...
describe('configure-action tests', () => {
  beforeEach(() => {
    ...
    process.env['INPUT_TOKEN'] = process.env['CLOUDTRUTH_TOKEN']
  ...
  it('configures the default environment', async () => {
    ...
    expect(spySetEnvvar).toHaveBeenCalledWith(
      'CTTEST_NOT_A_SECRET',
      'not_a_secret_default'
    )
    expect(spySetEnvvar).toHaveBeenCalledWith(
      'CTTEST_TOTALLY_A_SECRET',
      'totally_a_secret_default'
    )
    expect(spySetEnvvar).toHaveBeenCalledWith(
      'CTTEST_HAS_NO_OVERRIDE',
      'has_no_override_default'
    )
    expect(spySetEnvvar).toHaveBeenCalledWith(
      'cttest.not.posix',
      'not.posix.default'
  ...
  it('configures the override environment', async () => {
    ...
    expect(spySetEnvvar).toHaveBeenCalledWith(
      'CTTEST_NOT_A_SECRET',
      'not_a_secret_override'
    )
    expect(spySetEnvvar).toHaveBeenCalledWith(
      'CTTEST_TOTALLY_A_SECRET',
      'totally_a_secret_override'
    )
    expect(spySetEnvvar).toHaveBeenCalledWith(
      'CTTEST_HAS_NO_OVERRIDE',
      'has_no_override_default'
  ...
  it('fails properly with an unknown project name', async () => {
    ...
    await run()
    expect(spyFailed).toHaveBeenCalledWith(
      `Project "not_a_real_project_name" not found.`
    )
  })
  ...
  it('fails properly with an unknown environment name', async () => {
    ...
    await run()
    expect(spyFailed).toHaveBeenCalledWith(
      `Environment "no_such_environment" not found.`
    )
  })
